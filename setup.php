	<div class="wrap">
		...
		<?php if (!empty($_POST['country-limit_result'])): ?><h3 id="country-limit_result" style="color:#ff0000;"><?php echo esc_html($_POST['country-limit_result']); ?></h3><?php endif; ?>
		...
		<div id="setting-error-settings_updated" class="updated settings-error">
			...
			<li style="margin:2px 0;">IPアドレス一覧の自動更新は<strong style="color:#0000ff;">「wp-cron.php」</strong>を使用します。「wp-cron.php」を使用しない場合はcronに<strong style="color:#0000ff;">「php <?php echo esc_html(dirname(__FILE__)); ?>/batch.php 1 &gt; /dev/null 2>&amp;1」</strong>を登録することで自動更新が可能です</li>
			<li style="margin:2px 0;">このプラグインはディレクトリ<strong style="color:#0000ff;">「WordPressのroot」「wp-content」「<?php echo esc_html(dirname(__FILE__)); ?>」</strong>に対して書込み権限が必要となります</li>
			<li style="margin:2px 0;">このプラグインをFTPなどでアップロードした場合は<strong style="color:#0000ff;">「<?php echo esc_html(dirname(__FILE__)); ?>/batch.php」</strong>対して実行権限が必要となる場合があります</li>
		...
		<?php wp_nonce_field('save', 'country-limit_update'); ?>
		...
		<tr style="background-color:#f9f9f9;">
			<td style="color:red;border-top:0;"><?php echo esc_html($_POST['COUNTRY_LIMIT_STATUS']['error']); ?></td>
		</tr>
		...
		<tr style="background-color:#ececec;">
			<td style="color:red;border-top:0;"><?php echo esc_html($_POST['COUNTRY_LIMIT_MTHOD']['error']); ?></td>
		</tr>
		<tr style="background-color:#f9f9f9;">
			...
			<td style="border-bottom:0;"><textarea name="COUNTRY_LIMIT_LIST[]" rows="5" cols="70" class="search-input"><?php echo esc_attr($_POST['COUNTRY_LIMIT_LIST'][0]); ?></textarea></td>
		</tr>
		<tr style="background-color:#f9f9f9;">
			<td style="color:red;border-top:0;"><?php echo esc_html($_POST['COUNTRY_LIMIT_LIST']['error']); ?></td>
		</tr>
		<tr style="background-color:#ececec;">
		...
		<tr style="background-color:#ececec;">
			<td style="color:red;border-top:0;"><?php echo esc_html($_POST['COUNTRY_LIMIT_TYPE']['error']); ?></td>
		</tr>
		<tr style="background-color:#f9f9f9;">
			<th rowspan="2" style="border-right:1px solid #dfdfdf;">上記の国以外でアクセスを拒否(又は許可)するIPアドレス<br />(127.0.0.1、127.0.0.1/32など)</th>
			<td style="border-bottom:0;"><textarea name="COUNTRY_LIMIT_EXTRA[]" rows="5" cols="70" class="search-input"><?php echo esc_attr($_POST['COUNTRY_LIMIT_EXTRA'][0]); ?></textarea></td>
		</tr>
		<tr style="background-color:#f9f9f9;">
			<td style="color:red;border-top:0;"><?php echo esc_html($_POST['COUNTRY_LIMIT_EXTRA']['error']); ?></td>
		</tr>
		<tr style="background-color:#ececec;">
			...
			<td style="border-bottom:0;">
				...
				<option value="<?php echo esc_html($i); ?>"<?php if ($_POST['COUNTRY_LIMIT_RENEW'][0] == $i): ?> selected="selected"<?php endif; ?>><?php echo esc_html($i); ?>日</option>
				<?php endforeach; ?>
				</select>
			</td>
		</tr>
		<tr style="background-color:#ececec;">
			<td style="color:red;border-top:0;border-bottom:0;"><?php echo esc_html($_POST['COUNTRY_LIMIT_RENEW']['error']); ?></td>
		</tr>
